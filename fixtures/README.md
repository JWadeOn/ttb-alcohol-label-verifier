...
**Programmatic verify (stress / regression):** `npm run eval:fixture-verify` with **`OPENAI_API_KEY`**, **`BASE_URL`**, and optional **`EVAL_FIXTURE_IDS`** (comma-separated manifest `id` values; defaults to **`difficult-synthetic-label-photo`**) or **`EVAL_FIXTURE_SET`** presets (`st_petersburg`, `edge_synthetic`, `seed_textures`, `all_manifest`). Set **`EVAL_OUT=docs/evals/…json`** to log a copy. See **`evals/run-fixture-verify.mjs`**.
